...
# Список типов агентов для анализа
AGENT_TYPES = [
    'modernresidentialbuilding',
    'mall',
    'hospitalbuilding',
    'officebuilding',
    'enterprisebuilding',
...
def analyze_contributions(df_agent, output_dir):
    ...
    Ограничено типами из AGENT_TYPES.
    """
    ensure_dir(output_dir)

    # Фильтрация по нужным типам
    df_filt = df_agent[df_agent['Type'].isin(AGENT_TYPES)]
    type_contrib = df_filt.groupby('Type')['Consumption'].sum()
...
def main():
    agent_path = os.path.join('simulations_results', 'agent_data_with_apartments.csv')
    ...
    # Сужаем набор агентов только до нужных типов
    df_agent = df_agent[df_agent['Type'].isin(AGENT_TYPES)]

    # 1) Общий расход энергии
    analyze_patterns(df_agent['Consumption'], 'total_consumption',
                     os.path.join('analysis_v2', 'Consumption'), 'sum')

    # 2) По заданным типам агентов (среднее)
    for agent_type in AGENT_TYPES:
        group = df_agent[df_agent['Type'] == agent_type]
        if group.empty:
            continue
        analyze_patterns(
            group['Consumption'],
            f"{agent_type}_consumption",
            os.path.join('analysis_v2', f'{agent_type}_consumption'),
            'mean'
        )

    # 3) Анализ параметров модели (не изменяем)
    for col in ['office_population', 'hospitalized', 'patients_total', 'presence_in_building']:
        analyze_patterns(df_model[col], col, os.path.join('analysis_v2', col), 'mean')

    # 4) Процентные изменения параметров модели
    pct = df_model[['office_population', 'hospitalized', 'patients_total']].pct_change().dropna()
    changes_dir = os.path.join('analysis_v2', 'changes')
    ensure_dir(changes_dir)
    pct.to_csv(os.path.join(changes_dir, 'model_params_pct_change.csv'))

    # 5) Анализ вклада только выбранных типов
    analyze_contributions(df_agent, os.path.join('analysis_v2', 'contribution'))

    print("Analysis completed. Результаты сохранены в папке 'analysis_v2'.")
